...
referred to as "the Manual".
     ...
     with the sole exceptions of the serial_out() function, and an
...
used for housekeeping, handshaking and module initialization.
   ...
   for (j=0; j<sizeof(sec) / sizeof(sec[0]); j++) {
     ...
     if(d0.flgs != 0) serial_out("Section %s returned %d.\n",d0.rfs,d0.rrc);
   }
  
   if(d0.crc == 0) serial_out("\nNo errors detected.\n");
   else serial_out("\nFailed.\n");
   return 0;
}
...
struct defs *pd0;
{
   ...
   if(a+_+_234+a234 != 10) {
     rc = rc+1;
     if(pd0->flgd != 0) serial_out(s22er,1);
   }
   ...
   if (A == a) {
     rc = rc+4;
     if (pd0->flgd != 0) serial_out(s22er,4);
   }
...
struct defs *pd0;
{
   ...
   if (   8 !=  010
     ...
     rc = rc+1;
     if( pd0->flgd != 0 ) serial_out(s241er,1);
   }
   ...
   if ( 0x00abcdef != 0xabcdef
     ...
     rc = rc+2;
     if( pd0->flgd != 0 ) serial_out(s241er,2);
   }
   ...
   if ( sizeof 010000000000 != sizeof(long)      /* 2**30 */
     ...
     rc = rc+4;
     if( pd0->flgd != 0 ) serial_out(s241er,4);
   }
   ...
   if ( sizeof   67l != sizeof(long)
     ...
     rc = rc+8;
     if( pd0 -> flgd != 0 ) serial_out(s241er,8);
   }
   ...
   for (j=0; j<39; j++){
     if ( g[j] != d[j]
       ...
       if( pd0 -> flgm != 0 ) {
/*       serial_out(s241er,16);          save in case opinions change...     */
         serial_out("Decimal and octal/hex constants sometimes give\n");
         serial_out("   different results when assigned to longs.\n");
       }
/*     lrc = 1;   save...   */
     }
   }
...
struct defs *pd0;
{
   ...
   if(sumof(chars) != 98){
     rc = rc+1;
     if(pd0->flgd != 0) serial_out(s243er,1);
   }
   ...
   if( '\0'    !=   0 || '\01'   !=   1 || '\02'   !=   2
    ...
    || '\100'  !=  64 || '\177'  != 127                 ){
     ...
     rc = rc+8;
     if(pd0->flgd != 0) serial_out(s243er,8);
   }
...
struct defs *pd0;
{
   ...
   if(lrc != 0) {
     if(pd0->flgd != 0) serial_out(s244er,1);
     rc = rc+1;
   }
   ...
   if ( (sizeof .1250E+04 ) != sizeof(double)
     ...
     if(pd0->flgd != 0) serial_out(s244er,2);
     rc = rc+2;
   }
...
struct defs *pd0;
{
   ...
   if ( s[0] != s[1] || s[1] != s[2]
    ...
    rc = rc+1;
     if(pd0->flgd != 0) serial_out(s25er,1);
   }
   ...
   if( s[3] != '\0' ){
     rc = rc+4;
     if(pd0->flgd != 0) serial_out(s25er,4);
   }
   ...
   /* In a string, the double quote character " must be preceded
    ...
    if( ".\"."[1] != '"' ){
    rc = rc+8;
     if(pd0->flgd != 0) serial_out(s25er,8);
   }
   ...
   if( s[0] != '\n'
    ...
    || s[6] != '\'' ){
     ...
     rc = rc+16;
     if( pd0->flgd != 0) serial_out(s25er,16);
   }
   ...
   if (lrc != 0){
     rc = rc+32;
     if(pd0->flgd != 0) serial_out(s25er,32);
   }
   return rc;
}
...
struct defs *pd0;
{
   ...
   if(pd0->flgm != 0) {
     serial_out(s,pd0->cbits,"char");
     serial_out(s,pd0->ibits,"int");
     serial_out(s,pd0->sbits,"short");
     serial_out(s,pd0->lbits,"long");
     serial_out(s,pd0->ubits,"unsigned");
     serial_out(s,pd0->fbits,"float");
     serial_out(s,pd0->dbits,"double");
     serial_out(s2,pd0->fprec,"float");
     serial_out(s2,pd0->dprec,"double");
   }
...
the block.                                                     */
   ...
   for (j=0; j<3; j++)
     if(svtest(j) != zero()){
       rc = 1;
       if(pd0->flgd != 0) serial_out(s4er,1);
     }
   ;
...
unication between functions, even separately compiled functions.
   ...
   if(testev() != 0){
     rc=rc+2;
     if(pd0->flgd != 0) serial_out(s4er,2);
   }
/*   
...
integers.
   ...
   if(sizeof lint < sizeof pint || sizeof pint < sizeof sint){
     ...
     rc = rc+4;
     if(pd0->flgd != 0) serial_out(s4er,4);
   }
...
implementation                                                  */
   ...
   if(mask != 1 || target != 0){
     ...
     rc = rc+8;
     if(pd0->flgd != 0) serial_out(s4er,8);
   }
...
involves sign extension.                                */
   ...
   if(to != -19){
     rc = rc+1;
     if(pd0->flgd != 0) serial_out(s61er,1);
   }
...
acter set is nonnegative.                               */
   ...
   if(lrc != 0){
     rc=rc+2;
     if(pd0->flgd != 0) serial_out(s61er,2);
   }
...
simply discarded.                                       */
   ...
   if((shortint != longint && shortint != 3) ||
     ...
     rc = rc+8;
     if(pd0->flgd != 0) serial_out(s61er,8);
   }
...
struct defs *pd0;
{
   ...
   if(f1>2.*pd0->fprec){
     ...
     rc = rc+2;
     if(pd0->flgd != 0) serial_out(s626er,2);
   }
   ...
   if(t0 != 0){
     ...
     if(pd0->flgd != 0){
       ...
       serial_out(s626er,4);
       serial_out("   key=");
       for(j=0;j<28;j++) serial_out("%d",t[j]);
       serial_out("\n");
     }
   }
   ...
   if((long)l > (unsigned)0100000){
      ...
      rc = rc+8;
      if(pd0->flgd != 0) serial_out(s626er,8);
   }
...
/*   A string is a primary expression. The identifier points to
   ...
   if(*"queep" != q){
     rc = rc+1;
     if(pd0->flgd  != 0) serial_out(s71er,1);
   }
/*   A parenthesized expression is a primary expression whose
   ...
   if((2+3) != 2+3) {
     rc = rc+2;
     if(pd0->flgd != 0) serial_out(s71er,2);
   }
...
/*   A primary expression followed by an expression in square 
   ...
   if(x[5] != 1942 || x[5] != *((x)+(5))){
     rc = rc+4;
     if(pd0->flgd != 0) serial_out(s71er,4);
   }
...
/*   If the various flavors of function calls didn't work, we
   ...
   if ( McCarthy(-5) != 91){
     rc = rc+8;
     if(pd0->flgd != 0) serial_out(s71er,8);
   }
...
/*   and that argument passing is strictly by value.           */
   ...
   if(a != 2 || b != 2){
     rc = rc+16;
     if(pd0->flgd != 0) serial_out(s71er,16);
   }
...
/*   Finally, structures and unions are addressed thusly:      */
   ...
   if(pd0->dprec != (*pd0).dprec){
     rc = rc+32;
     if(pd0->flgd != 0) serial_out(s71er,32);
   }
...
initial         (5,2)       |    (5,2)    |  (12,10)
   ...
   if(lrc != 0) {
     rc = 1;
     if(pd0->flgd != 0) serial_out(s714er,1);
   }
   return rc;
}
...
struct defs *pd0;
{
   ...
   i = 1;
   if( i++,i++,i++,i++,++i != 6 ){
     if(pd0->flgd != 0) serial_out(s715er,1);
     rc = rc+1;
   }
   ...
   if(s715f(a, (t=3, t+2), c) != 5){
     if(pd0->flgd != 0) serial_out(s715er,2);
     rc = rc+2;
   }
   return rc;
}
...
struct defs *pd0;
{
   ...
   if(t<<3<<2 != 1280 || t>>3>>2 != 1){
     rc = rc+1;
     if(pd0->flgd != 0) serial_out(s757er,1);
   }
   ...
   if(lrc != 0){
     rc = rc+2;
     if(pd0->flgd != 0) serial_out(s757er,2);
   }
   ...
   if((a<b<c) != 1){
     rc = rc+4;
     if(pd0->flgd != 0) serial_out(s757er,4);
   }
   ...
   if( &x[1] == &x[0] ){
     rc = rc+8;
     if(pd0->flgd != 0) serial_out(s757er,8);
   }

   if( &x[1] < &x[0] ) if(pd0->flgm != 0)
     serial_out("Increasing array elements assigned to decreasing locations\n");
   ...
   if(lrc != 0){
     rc = rc+16;
     if(pd0->flgd != 0) serial_out(s757er,16);
   }
   ...
   if(p != 0){
     rc = rc+32;
     if(pd0->flgd != 0) serial_out(s757er,32);
   }
...
struct defs *pd0;
{
   ...
   if(((int)12 ^ (unsigned)10) !=  6) {lrc = 72;
      if(prlc) serial_out(fl,lrc);}
   if(((int)12 & (int)10) !=  8) {lrc = 73; if(prlc) serial_out(fl,lrc);}
   if(((int)12 | (int)10) != 14) {lrc = 74; if(prlc) serial_out(fl,lrc);}
   if(((int)12 ^ (int)10) !=  6) {lrc = 75; if(prlc) serial_out(fl,lrc);}

   if(lrc != 0){
     if(pd0->flgd != 0) serial_out(s7813er,1);
     rc = rc+1;
   }
   ...
   r = i++ && j++;
    if(i!=1) {lrc = 1; if(prlc) serial_out(fl,lrc);}
    if(j!=0) {lrc = 2; if(prlc) serial_out(fl,lrc);}
    if(r!=0) {lrc = 3; if(prlc) serial_out(fl,lrc);}
   r = i && j++;
    if(i!=1) {lrc = 4; if(prlc) serial_out(fl,lrc);}
    if(j!=1) {lrc = 5; if(prlc) serial_out(fl,lrc);}
    if(r!=0) {lrc = 6; if(prlc) serial_out(fl,lrc);}
   r = i-- && j;
    if(i!=0) {lrc = 7; if(prlc) serial_out(fl,lrc);}
    if(j!=1) {lrc = 8; if(prlc) serial_out(fl,lrc);}
    if(r!=1) {lrc = 9; if(prlc) serial_out(fl,lrc);}
   r = i && j--;
    if(i!=0) {lrc = 10; if(prlc) serial_out(fl,lrc);}
    if(j!=1) {lrc = 11; if(prlc) serial_out(fl,lrc);}
    if(r!=0) {lrc = 12; if(prlc) serial_out(fl,lrc);}

   if(lrc!=0){
     if(pd0->flgd != 0) serial_out(s7813er,2);
     rc = rc+2;
   }
   ...
   r = j++ || i;
    if(i!=1) {lrc = 4; if(prlc) serial_out(fl,lrc);}
    if(j!=1) {lrc = 5; if(prlc) serial_out(fl,lrc);}
    if(r!=1) {lrc = 6; if(prlc) serial_out(fl,lrc);}
   r = i-- || j--;
    if(i!=0) {lrc = 7; if(prlc) serial_out(fl,lrc);}
    if(j!=1) {lrc = 8; if(prlc) serial_out(fl,lrc);}
    if(r!=1) {lrc = 9; if(prlc) serial_out(fl,lrc);}
   r = i || j--;
    if(i!=0) {lrc = 10; if(prlc) serial_out(fl,lrc);}
    if(j!=0) {lrc = 11; if(prlc) serial_out(fl,lrc);}
    if(r!=1) {lrc = 12; if(prlc) serial_out(fl,lrc);}

   if(lrc!=0){
     if(pd0->flgd != 0) serial_out(s7813er,4);
     rc = rc+4;
   }
   ...
   r = one?zero:one?i++:j++;
   if(r!=0 || i!=0 || j!=0){
     if(pd0->flgd != 0) serial_out(s7813er,8);
     rc = rc+8;
   }
   ...
   if((one?zero:1) != 0 || (zero?1:zero) != 0){
     if(pd0->flgd != 0) serial_out(s7813er,16);
     rc = rc+16;
   }
   return rc;
}
...
/*    The storage class specifiers are:
   ...
   if ( crc != 0 ) {
     rc = rc+1;
     if( pd0 -> flgd != 0 ) serial_out(s81er,1);
   }

   if ( prc != 0 ) {
     rc = rc+2;
     if( pd0 -> flgd != 0 ) serial_out(s81er,2);
   }

   if ( irc != 0 ) {
     rc = rc+4;
     if( pd0 -> flgd != 0 ) serial_out(s81er,4);
   }
...
/*   Now we check to see if variables are actually being assigned
   ...
   k = regc();
   if ( pd0->flgm != 0 ) {
     if ( k < 0 ) serial_out(badtest,"char");
     else serial_out(goodtest,k,"char");
   }

   k = regp();
   if ( pd0->flgm != 0 ) {
     if ( k<0 ) serial_out(badtest,"pointer");
     else serial_out(goodtest,k,"pointer");
   }

   k = regi();
   if ( pd0->flgm != 0 ) {
     if ( k<0 ) serial_out(badtest,"int");
     else serial_out(goodtest,k,"int");
   }
...
struct defs *pd0;
{
   ...
   ip = fip(3);
   if(*ip != 3){
     if(pd0->flgd != 0) serial_out(s84er,1);
     rc = rc+1;
   }

   pfi = glork;
   if((*pfi)(4) != 4){
     if(pd0->flgd != 0) serial_out(s84er,2);
     rc = rc+2;
   }
   ...
   if(sum != 136){
     if(pd0->flgd != 0) serial_out(s84er,4);
     rc = rc+4;
   }
   ...
   if( array(x3d,105,0)
      ...
      if(pd0->flgd != 0) serial_out(s84er,8);
      rc = rc+8;
   }
...
struct defs *pd0;
{
   ...
   if( (char *)&s1.count - &s1.tword[0] <= 0
     ...
     if(pd0->flgd != 0) serial_out(s85er,1);
     rc = rc+1;
   }
   ...
   if(s3.threebit != 3){
     if(s3.threebit == -1){
       if(pd0->flgm != 0) serial_out("Sign extension in fields\n");
     }
     else{
       if(pd0->flgd != 0) serial_out(s85er,2);
       rc = rc+2;
     }
   }
   ...
   if(s3.onebit != 1){
     if(pd0->flgm != 0)
      serial_out("Be especially careful with 1-bit fields!\n");
   }
   ...
   if( (char *)u0.u1 - (char *)&u0 != 0
     ...
     if(pd0->flgd != 0) serial_out(s85er,4);
     rc = rc+4;
   }
   ...
   if( sizeof u0 < sizeof u0.u1
     ...
     if(pd0->flgd != 0) serial_out(s85er,8);
     rc = rc+8;
   }
   ...
   s1.right->tword[0] += 1;
   if(s2.tword[0] != 3){
     if(pd0->flgd != 0) serial_out(s85er,16);
     rc = rc+16;
   }
   return rc;
}
...
struct defs *pd0;
{
   ...
   if(*pint != 5){
     if(pd0->flgd != 0) serial_out(s86er,1);
     rc = rc+1;
   }
   ...
   if(*apint != 3){
     if(pd0->flgd != 0) serial_out(s86er,2);
     rc = rc+2;
   }

   if(*rpint != 5){
     if(pd0->flgd != 0) serial_out(s86er,4);
     rc = rc+4;
   }
   ...
   for(j=0; j<10; j++)
     if(zero[j] != 0) lrc = 1;
   if(lrc != 0){
     if(pd0->flgd != 0) serial_out(s86er,8);
     rc = rc+8;
   }
   ...
   if(lrc != 0){
     if(pd0->flgd != 0) serial_out(s86er,16);
     rc = rc+16;
   }
   ...
   if(lrc != 0){
     if(pd0->flgd != 0) serial_out(s86er,32);
     rc = rc+32;
   }
   return rc;
}
...
struct defs *pd0;
{
   ...
   if(sizeof distance != sizeof(int)){
     if(pd0->flgd != 0) serial_out(s88er,1);
     rc = rc+1;
   }
   ...
   *metricp = 3;
   ...
   if(distance != 3){
     if(pd0->flgd != 0) serial_out(s88er,2);
     rc = rc+2;
   }
   ...
   zp->im = 1.;
   if(z.re+z.im != 2.){
     if(pd0->flgd != 0) serial_out(s88er,4);
     rc = rc+4;
   }
...
struct defs *pd0;
{
   ...
   if(lrc != 0){
     if(pd0->flgd != 0) serial_out(s9er,1);
     rc = rc+1;
   }
   ...
   goto nobarf;
     if(pd0->flgd != 0) serial_out(s9er,2);
     rc = rc+2;
   nobarf:;
