...
//
branch::branch(const string word)
{
    _word = word;
...
//
void branch::add(const string word)
{
    // Implement something along these lines:
    // compare word wiconst th _word using word->compare(_word->data());
...
//
branch* branch::find(const string word)
{
	int i;
	i = word.compare(_word);
...
//
branch* branch::remove(const string word, branch** ptr_to_branch_ptr)
{
	//pointer update ()
	//reshuffle ()
